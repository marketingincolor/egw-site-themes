 ...
 */
if (!class_exists('DiscussionStickyNavigationWalker')) {
	class DiscussionStickyNavigationWalker extends Walker_Nav_Menu {
		...
		function start_lvl( &$output, $depth = 0, $args = array() ) {
			...
			if($depth == 0){
				$out_div = '<div class="mkd-menu-second"><div class="mkd-menu-inner">';
			}else{
				$out_div = '';
			}
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if($depth==0){
				if($item->type_menu == "wide") {
					if($item->icon != "" && $item->icon != 'null'){ 
						$menu_type = " mkd-menu-wide mkd-menu-wide-with-icons";
					} else { 
						$menu_type = " mkd-menu-wide";
					}
				} elseif ($item->type_menu == "wide_custom_widget"){
					$menu_type = " mkd-menu-wide mkd-menu-custom-widget";
				} else {
					$menu_type = " mkd-menu-narrow";
				}
			}
			...
			if($depth==0 && $item->type_menu != "wide_custom_widget"){
				if($item->wide_position == "right"){
					$wide_menu_position = " mkd-menu-right-position";
				}elseif($item->wide_position == "left"){
					$wide_menu_position = " mkd-menu-left-position";
				}else{
					$wide_menu_position = "";
				}
			}
			...
			if($item->hide == ""){
				...
				$angle_icon   = '';
				if($depth !== 0) {
					$angle_icon = '<i class="mkd_menu_arrow ion-ios-arrow-right"></i>';
				}
				...
				if($args->has_children && $depth == 0) {
					$item_output .= '<span class="mkd_menu_arrow ion-ios-arrow-down"></span>';
				}
