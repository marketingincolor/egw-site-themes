...
abstract class ListShortcode {
    ...
    public function vcMap() {
        if(function_exists('vc_map')) {
            vc_map( array(
                ...
                "params" => discussion_get_shortcode_params($this->base)
            ) );
        }
    ...
    public function renderHolders($atts, $content = null) {
        ...
        $args = discussion_get_shortcode_params_names(discussion_get_shortcode_params($this->base));
		...
		if (empty($params['number_of_posts'])){
			switch ($this->base) {
				...
				case 'mkd_block_seven':
					$params['number_of_posts'] = '3';
					break;
				default:
					$params['number_of_posts'] = '6';
					break;
			}
		}
        ...
        $html .= '<div class="mkd-bnl-holder '.$this->css_class.'-holder  '.esc_attr($params['holder_classes']).'" '.discussion_get_inline_attrs($params['posts_data']).'>';
		if (!empty($params['title']) && $params['title'] !== ''){
			$pattern_style = '';
			if (!empty($params['pattern_style'])){
				$pattern_style = $params['pattern_style'];
			}

			$html .= '<div class="mkd-layout-title-holder">';
			$html .= discussion_execute_shortcode('mkd_section_title',array('title' => $params['title'],'pattern_style' => $pattern_style));
			$html .= '</div>';
		}
    ...
    public function generatePostsQuery($params) {
        $queryResult = discussion_get_query($params);
    ...
    private function generateImageSize($params, $type) {
        ...
        if ($imageSize !== '' && $imageSize == 'landscape') {
            $thumbImageSize = 'discussion_landscape';
        } else if($imageSize !== '' && $imageSize === 'square'){
            $thumbImageSize = 'discussion_square';
        } else if ($imageSize !== '' && $imageSize == 'portrait') {
            $thumbImageSize = 'discussion_portrait';
    ...
    protected function errorMessage() {
        return '<div class="'.$this->css_class.'-message"><p>'.esc_html__('No posts were found.', 'discussionwp').'</p></div>';
    }
    ...
    private function getPaginationHtml($params, $atts) {
        ...
        switch($params['pagination_type']){
            case 'load-more' : {
                $html .= discussion_get_list_shortcode_module_template_part('post-show-load-more-template', 'templates', '', $atts);
            } break;
            case 'np-horizontal' : {
                $html .= discussion_get_list_shortcode_module_template_part('post-direction-nav-horizontal-template', 'templates', '', $atts);
                $html .= discussion_get_list_shortcode_module_template_part('post-ajax-preloader-template', 'templates', '', $atts);
            } break;
            default : break;
        }
