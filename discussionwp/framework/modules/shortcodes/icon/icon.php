<?php
namespace Discussion\Modules\Shortcodes\Icon;

use Discussion\Modules\Shortcodes\Lib\ShortcodeInterface;

/**
 * Class Icon
 * @package Discussion\Modules\Shortcodes\Icon
 */
class Icon implements ShortcodeInterface {
    ...
    public function vcMap() {
        vc_map(array(
            'name'                      => esc_html__('Mikado Icon', 'discussionwp'),
            'base'                      => $this->base,
            'category'                  => 'by MIKADO',
            'icon'                      => 'icon-wpb-icons extended-custom-icon',
            'allowed_container_element' => 'vc_row',
            'params'                    => array_merge(
                \DiscussionIconCollections::get_instance()->getVCParamsArray(),
                array(
                    array(
                        'type'        => 'dropdown',
                        'heading'     => 'Size',
                    ...
                    array(
                        ...
                        'description' => esc_html__('Please insert border radius(Rounded corners) in px. For example: 4 ', 'discussionwp'),
                        'dependency'  => array('element' => 'type', 'value' => array('square','landscape'))
                    ),
                    array(
                        'type'        => 'textfield',
                    ...
                    array(
                        'type'        => 'textfield',
                        'heading'     => 'Shape Width (px)',
                        'param_name'  => 'shape_width',
                        'admin_label' => true,
                        'value'       => '',
                        'description' => '',
                        'dependency'  => array('element' => 'type', 'value' => array('landscape'))
                    ),
                    array(
                        'type'        => 'textfield',
                        'heading'     => 'Shape Height (px)',
                    ...
                    array(
                        ...
                        'admin_label' => true,
                        'dependency'  => array('element' => 'type', 'value' => array('circle', 'square', 'landscape'))
                    ),
                    array(
                        'type'        => 'textfield',
    ...
    public function render($atts, $content = null) {
        ...
        $default_atts = array_merge($default_atts, discussion_icon_collections()->getShortcodeParams());
        $params       = shortcode_atts($default_atts, $atts);

        $iconPackName = discussion_icon_collections()->getIconCollectionParamNameByKey($params['icon_pack']);
        ...
        $html = discussion_get_shortcode_module_template_part('templates/icon', 'icon', '', $params);
    ...
    private function generateIconStyles($params) {
        ...
        if(!empty($params['custom_size'])) {
            $iconStyles[] = 'font-size:'.discussion_filter_px($params['custom_size']).'px';
        }
    ...
    /**
     * Generates icon holder styles for circle, square and landscape icon type
     *
     * @param $params
     *
     * @return array
    ...
    private function generateIconHolderStyles($params) {
        ...
        if(isset($params['type']) && $params['type'] !== 'normal') {
			...
			if(!empty($params['custom_size'])) {
				$iconHolderStyles[] = 'font-size:'.discussion_filter_px($params['custom_size']).'px';
			}
			...
			if ($params['type'] == 'landscape'){
				if ($params['shape_height'] !== ''){
					$iconHolderStyles[] = 'height: '.discussion_filter_px($params['shape_height']).'px';
					$iconHolderStyles[] = 'line-height: '.discussion_filter_px($params['shape_height']).'px';
				}
				if ($params['shape_width'] !== ''){
					$iconHolderStyles[] = 'width: '.discussion_filter_px($params['shape_width']).'px';
				}
			}
			else{
                if(!empty($shapeSize)) {
                    $iconHolderStyles[] = 'width: '.discussion_filter_px($shapeSize).'px';
                    $iconHolderStyles[] = 'height: '.discussion_filter_px($shapeSize).'px';
                    $iconHolderStyles[] = 'line-height: '.discussion_filter_px($shapeSize).'px';
                }
            }
            ...
            if(!empty($params['border_color']) && (isset($params['border_width']) && $params['border_width'] !== '')) {
				...
				$iconHolderStyles[] = 'border-width: '.discussion_filter_px($params['border_width']).'px';
            ...
            if($params['type'] == 'square' || $params['type'] == 'landscape') {
                if(isset($params['border_radius']) && $params['border_radius'] !== '') {
                    $iconHolderStyles[] = 'border-radius: '.discussion_filter_px($params['border_radius']).'px';
                }
            }
        }
